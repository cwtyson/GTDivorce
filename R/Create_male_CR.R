...
## Remove individuals that have breeding records where the order cannot be determined
male_filter <- pair_dat %>%
  filter(no_order_m == "yes") %>%
  distinct(male) %>%
  pull(male)

## Read in pair data
pair_dat <- pair_dat %>%
  filter(!(male %in% male_filter))
...
for(pop_code in unique(pair_dat$pop)){
  ...
  pair_dat_complete <- pair_dat %>%
    ...
    arrange(pop, male, year, order) %>%
    group_by(pop, male) %>%
    mutate(multi_partner = case_when(any(m_pair_num == 2) ~ 1,
                                     TRUE ~ 0),
           prev_partner = lag(female)) %>%
    ...
    distinct(pop, male, year, .keep_all = T) %>%
    select(pop, male, year, multi_partner, event_code)


  ## Pivot wider
  pair_dat_wide <- pair_dat_complete %>%
    pivot_wider(id_cols = c(pop, male, multi_partner),
                names_from = year,
  ...
  write_csv(x = pair_dat_wide,
            file = paste0(here::here(),
                          paste("/data/event_codes/m/", pop_code, "_male_event_codes.csv", sep = "")))
